...
## Canvas drag coordinate correction — 2026-09-20

The reported Canvas card jumping was reproduced before modifying the drag implementation. A new regression moves a synthetic pointer along a fixed Canvas-space path and allows GTK to allocate the changed widgets between samples. On the second move, the old implementation placed the card at approximately `(30, 29)` instead of `(54, 41)`, even though the pointer continued forward (`target/qa/canvas-drag-before.log`). Earlier smoke checks emitted only one update before ending a drag, so they did not exercise this feedback.

Gesture offsets belong to the widget hosting the controller. The old handler added those offsets directly to the original Canvas position; moving the card or resizing its handle changed the coordinates used by the next event. The corrected handler records the press in both widget and Canvas coordinates, converts each current point into Canvas space, and computes displacement there before applying snapping and bounds. See GTK's [coordinate-system documentation](https://docs.gtk.org/gtk4/coordinates.html) and [compute_point API](https://docs.gtk.org/gtk4/method.Widget.compute_point.html).

Resize calculations now start from the full card bounds, including the CSS border, and keep the unmodified axis at its original size. Reusing content width/height as the next whole-widget size could otherwise progressively shrink the other axis. Keyboard resizing and movement bounds use the same border box. Only the current gesture can update or clear drag ownership; a cancellation from another controller cannot clear it, and late motion after release is ignored. This change is limited to the shared Canvas interaction used by the embedded and detached selected-theme playgrounds; no decoder, theme asset, desktop setting or dependency was changed.

The new GUI regression covers both card bodies, the Cover title, right/bottom/corner resizing, repeated forward movement, a stationary sample, reverse movement, and grid snapping off/on (12 paths, eight motion samples each). It checks independent Canvas-space positions after GTK allocations, preserves the untouched resize axis, and checks unrelated cancellation plus late motion after release. These are synthetic signals with actual widget allocations, not injected hardware events or proof of compositor pixels.

Intermediate validation found two harness issues: the controller model advertises GObject rather than EventController, and the reset at the end of the regression requires a layout turn before the following screenshot. Both were corrected; logs are retained in `target/qa/canvas-drag-harness-initial.log` and `target/qa/canvas-drag-snapshot-timing.log`. A subsequent high-contrast resize failure is retained in `target/qa/canvas-drag-highcontrast-failure.log`; full border-box sizing and the preserved axis are now checked instead of feeding changing content dimensions into the expectation.

Real GNOME acceptance remains pending: physically drag both Canvas cards forward, pause, reverse, test the three resize handles with grid snapping on/off, and repeat after changing Canvas Scale or popping out the window. Check that no jump or visible trail persists after release. Geometry assertions and snapshots do not certify the compositor's incremental redraw, actual pointer appearance, fractional/mixed-monitor behavior, or post-close recovery. Existing Apply/Undo and external-conflict acceptance gates remain separate. No push or release was performed.

Final automated validation: PASS — `./scripts/check.sh`, exit 0 (`target/qa/canvas-drag-final-check.log`), including formatting, locked core/app tests, workspace Clippy with warnings denied, all 79 workspace tests and the release build. Local toolchain: Rust 1.98.0, GTK 4.22.4.

Final isolated GUI validation: PASS — `./scripts/check-gui.sh`, exit 0 (`target/qa/canvas-drag-gui.log`), Adwaita, Adwaita dark and HighContrast on the available Wayland display, reported scale 2. All three runs completed the new 96-sample drag regression and the existing trial, frontend, import, current-system and no-settings-write checks. Final logs contain no application GTK/GDK warning, critical or protocol error; private portal service/shutdown warnings are separate. X11/XWayland and physical-pointer/compositor trail acceptance were not rerun. The release binary is rebuilt; an already-running application must be closed and relaunched to use it.
